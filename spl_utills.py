...
class TrainingScheduler:
    ...
    def get_next_ratio(self):#########self.type默认'linear'，init_ratio默认0.1，max_thresh=2，grow_steps=10
        if self.type == 'const':
            ratio = self.init_ratio
        elif self.type == 'linear':
            ratio = self.init_ratio + (self.max_thresh - self.init_ratio) / self.grow_steps * self.step
            ######这里和公式（4）是一样的########print(ratio)1.05
...
def calculate_spl_weights(x, thresh, args, eps=1e-1):##########args.spl_type 默认'welsch'，开始权重会放得很小，是为了给难数据的加挑战

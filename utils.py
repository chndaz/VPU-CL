...
Returns:
    ...
    n_up = int(n_u * prior)  # number of positive samples in unlabeled data#####n_up表示的是未标记中正类数
    ...
    X_pre, y_pre, ty_pre = X_pre[perm], y_pre[perm], ty_pre[perm]#######X_pre 都是正类数据，X_u是传统的无标记类，（x_u前面是未标记正的后面是未标记负的）tr_pre是真实的

    print(
        f'labeled: {n_lp}    unlabeled: {n_u}    unlabeled positive: {n_up}    unlabeled negative: {n_un}    pos_targets: {pos_targets}    prior: {prior}')
    return X_lp, y_lp, X_u, y_u, X_pre, y_pre, ty_pre########y_pre里面标记‘有错误’，ty_pre是真实的label
...
Returns:
    ...
    lp_dataset = PUDataset(X_lp, y_lp)#########有有标记正类
    u_dataset = PUDataset(X_u, y_u)#########有有标记正类，有无标记正类，y_u是有正有负
